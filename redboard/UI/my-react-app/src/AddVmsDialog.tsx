...
// Function to open the database and create object store if it doesn't exist
...
// Function to store a task ID in IndexedDB
async function storeTaskId(taskId) {
  const db = await openDatabase();
  await db.put('tasks', { id: taskId });
  console.log(`Stored task ID: ${taskId}`);
}
...
const AddVmsDialog: React.FC<Props> = ({ open, handleClose, setTaskIds }) => {
    ...
    const handleAddNode = async () => {
      try {
        ...
        if (data.task_id) {
          await storeTaskId(data.task_id);
          setTaskIds(prevTaskIds => [...prevTaskIds, data.task_id]); // Update state immediately
